...
例如：FriendMessage的事件链为`['FriendMessage', 'MessageEvent', 'Event']`。
...
模型事件总线暂时不支持自定义事件。不过可以通过 `base_bus` 发送自定义事件。

```python
@mbus.base_bus.on('CustomEvent')
def handle_custom_event(x, y, z):
    print('CustomEvent', x, y, z)

await mbus.base_bus.emit('CustomEvent', 1, 2, 3)
```
